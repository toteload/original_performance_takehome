...
class KernelBuilder:
    ...
    def build_batch_step(self, round, i, val, node_val, idx, tmp1, tmp2, const_hash_stage, vconst):
        ...
        # val = val ^ node_val
        insts.append({"valu": [("^", val, val, node_val)]})

        # val = val * 4097 + 0x7ED55D16
        insts.append({"valu": [("multiply_add", val, val, const_hash_stage[0][1], const_hash_stage[0][0])]})
        insts.append({"debug": [("vcompare", val, [(round, i + j, "hash_stage", 0) for j in range(VLEN)])]})

        insts.append({"valu": [("^",  tmp1, val, const_hash_stage[1][0]),
                               (">>", tmp2, val, const_hash_stage[1][1])]})
        insts.append({"valu": [("^", val, tmp1, tmp2)]})
        insts.append({"debug": [("vcompare", val, [(round, i + j, "hash_stage", 1) for j in range(VLEN)])]})

        # val = (val << 5) + (val + 0x165667B1)
        # val = val * 33 + 0x165667B1
        insts.append({"valu": [("multiply_add", val, val, const_hash_stage[2][1], const_hash_stage[2][0])]})
        insts.append({"debug": [("vcompare", val, [(round, i + j, "hash_stage", 2) for j in range(VLEN)])]})

        # val = (val << 9) ^ (val + 0xD3A2646C)
        insts.append({"valu": [("+",  tmp1, val, const_hash_stage[3][0]),
                               ("<<", tmp2, val, const_hash_stage[3][1])]})
        insts.append({"valu": [("^", val, tmp1, tmp2)]})
        insts.append({"debug": [("vcompare", val, [(round, i + j, "hash_stage", 3) for j in range(VLEN)])]})

        # val = (val << 3) + (val + 0xFD7046C5)
        # val = val * 9 + 0xFD7046C5
        insts.append({"valu": [("multiply_add", val, val, const_hash_stage[4][1], const_hash_stage[4][0])]})
        insts.append({"debug": [("vcompare", val, [(round, i + j, "hash_stage", 4) for j in range(VLEN)])]})

        insts.append({"valu": [("^",  tmp1, val, const_hash_stage[5][0]),
                               (">>", tmp2, val, const_hash_stage[5][1])]})
        insts.append({"valu": [("^", val, tmp1, tmp2)]})
        insts.append({"debug": [("vcompare", val, [(round, i + j,   "hash_stage", 5) for j in range(VLEN)])]})

        insts.append({"debug": [("vcompare", val, [(round, i + j,   "hashed_val") for j in range(VLEN)])]})

        # idx = 2*idx + (1 if val % 2 == 0 else 2)
        # idx = 2*idx + (0 if val % 2 == 0 else 1) + 1
        # idx = (2*idx) + (val & 1) + 1

        insts.append({"valu": [("&", tmp2, val, vconst["1"]),
                               ("multiply_add", idx, idx, vconst["2"], vconst["1"])]})
        insts.append({"valu": [("multiply_add", idx, idx, vconst["1"], tmp2)]})
        insts.append({"debug": [("vcompare", idx, [(round, i + j, "next_idx") for j in range(VLEN)])]})

        # idx = 0 if idx >= n_nodes else idx
        insts.append({"valu": [("<", tmp1, idx, vconst["n_nodes"])]})
        insts.append({"flow": [("vselect", idx, tmp1, idx, vconst["0"])]})
        insts.append({"debug": [("vcompare", idx, [(round, i + j, "wrapped_idx") for j in range(VLEN)])]})
